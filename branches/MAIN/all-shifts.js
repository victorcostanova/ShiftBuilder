...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const searchInputs = document.querySelectorAll('#searchWorker, #searchWorkplace, #dateFrom, #dateTo');
...
function displayShifts(shifts) {
    ...
    if (shifts.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" class="no-data">No shifts found</td></tr>';
        return;
    }
    ...
    sortedShifts.forEach(shift => {
        ...
        // Add click event to navigate to edit page (using shift details)
        row.style.cursor = 'pointer';
        row.addEventListener('click', function() {
            // Navigate to the shift edit page for that specific user
            window.location.href = `add-shift.html?slug=${encodeURIComponent(shift.slug)}&user=${encodeURIComponent(shift.username)}`;
        });
...
function filterShifts() {
    const searchWorker = document.getElementById('searchWorker').value.toLowerCase().trim();
    const searchWorkplace = document.getElementById('searchWorkplace').value.toLowerCase().trim();
    ...
    if (dateFrom) {
        filteredShifts = filteredShifts.filter(shift =>
            new Date(shift.date) >= new Date(dateFrom)
        );
    }

    if (dateTo) {
        filteredShifts = filteredShifts.filter(shift =>
            new Date(shift.date) <= new Date(dateTo)
        );
    }
