...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const searchInputs = document.querySelectorAll('#searchShift, #dateFrom, #dateTo');
...
function displayShifts(shifts) {
    ...
    if (shifts.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" class="no-data">No shifts found. Click "+ Add Shift" to get started!</td></tr>';
        return;
    }
    ...
    sortedShifts.forEach(shift => {
        ...
        row.innerHTML = `
            ...
            <td class="action-buttons">
                <button class="btn btn-primary" onclick="editShift('${shift.slug}')">Edit</button>
                <button class="btn btn-danger" onclick="deleteShift('${shift.slug}')">Delete</button>
            </td>
        `;
...
function filterShifts() {
    const searchShift = document.getElementById('searchShift').value.toLowerCase().trim();
