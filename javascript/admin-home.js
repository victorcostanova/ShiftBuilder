...
function getAllUsers() {
  ...
  return Object.entries(users)
    .filter(([username, userData]) => username !== ADMIN_CREDENTIALS.username) // Exclude fixed admin account
...
function calculateWorkerOfMonth(allShifts) {
  ...
  document.getElementById("workerOfMonth").textContent =
    topWorker || "No data available";
  document.getElementById(
    "workerShiftCount"
  ).textContent = `${maxShifts} shift${maxShifts !== 1 ? "s" : ""}`;
}
...
function displayWeeklyShifts(allShifts) {
  ...
  if (weeklyShifts.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="6" class="no-data">No shifts this week</td></tr>';
    return;
  }

  // Sort by date (most recent first)
  const sortedShifts = weeklyShifts.sort(
